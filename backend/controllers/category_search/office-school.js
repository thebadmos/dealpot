  ...
  const kara = async (page) =>{
    let data = [];
      try {
          ...
          let $ = cheerio.load(response.data);
              data = [...karaSearchHtml($)];
          // response = await axios.get(`https://www.kara.com.ng/home-office-devices/?p=${page}`)
...
const pointekOnline = async (page) =>{
  let data = [];
    try {
        let response = await axios.get(`https://www.pointekonline.com/product-category/printers/page/${page}`)
        let $ = cheerio.load(response.data);
